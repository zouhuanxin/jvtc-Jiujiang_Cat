...
class now_booking extends StatefulWidget{
  @override
  State<StatefulWidget> createState() => now_booking_State();

}

class now_booking_State extends State<now_booking>{
  ...
  List<Widget> now_booking_list=[];
  List<dynamic>now_booking_data=[];

  void _query_now_booking() async{
    now_booking_data.clear();
    SharedPreferences sharedPreferences=await SharedPreferences.getInstance();
    String str1=await HttpUtil.now_library_book_query('tsqxydlbinfo', sharedPreferences.getString('tscookie'));
    if(json.decode(str1)['code'].toString().trim()=='0'){
      now_booking_data=json.decode(json.encode(json.decode(str1)['data']));
    }
  ...
  Widget table1(){
    return new Container(
      child: new Column(
        children: now_booking_list,
      ),
    );
  }

  void loading_table1_data(){
    now_booking_list.clear();
    for(var i=1;i<now_booking_data.length;i++){
      Map<String,Object>tempmap=json.decode(json.encode(now_booking_data[i]));
      now_booking_list.add(
          Container(
            padding: EdgeInsets.all(10.0),
            decoration: BoxDecoration(
              ...
              children: <Widget>[
                ...
                new Row(
                  children: <Widget>[
                    ...
                    new Text('预约(到书)日',textAlign: TextAlign.center),
                    Expanded(
                      flex: 1,
                ...
                new Row(
                  children: <Widget>[
                    new Text('截止日期',textAlign: TextAlign.center),
                    Expanded(
                      flex: 1,
                  ...
                  children: <Widget>[
                    ...
                    Expanded(
                      ...
                      child: new GestureDetector(
                        onTap: (){_cancel_booking(tempmap);},
                        child: new Text('取消预约',textAlign: TextAlign.center,overflow: TextOverflow.ellipsis,style: TextStyle(color: Colors.blue),),
                      ),
                    ),
                  ],
                )
  ...
  void _cancel_booking(Map<String,Object>tempmap) async{
    SharedPreferences sharedPreferences=await SharedPreferences.getInstance();
    String str1=await HttpUtil.cancel_booking('tsqxydinfo',
        tempmap['call_no'].toString().trim(),
        tempmap['marc_no'].toString().trim(),
        tempmap['loca'].toString().trim(), sharedPreferences.getString('tscookie'));
    if(json.decode(str1)['code'].toString().trim()=='0'){
