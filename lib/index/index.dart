...
class IndexState extends State<Index> with TickerProviderStateMixin{
  ...
  void _load_default_color() async{
   // print('_load_default_color');
    ...
    if(sharedPreferences.getString('login_state')=='true') {
      login_state=true;
      now_login_image_base64=sharedPreferences.getString('now_login_image_base64');
  ...
  bmob_get_app_Version_information(BuildContext context,note){
    ...
    query.queryObjects().then((data) {
      ...
      if(app_version.toString().trim()!=sfs[sfs.length-1].gxversion.toString().trim()){
        showmodel(context,'程序更新啦', sfs[sfs.length-1].text,sfs[sfs.length-1].url);
      }else{
        if(note=='my'){
    ...
    showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          actions: <Widget>[
            ...
            new FlatButton(
              child: new Text("更新",style: TextStyle(color: Colors.red),),
              onPressed: () {
                launchURL(url);
                Navigator.of(context).pop();
              },
