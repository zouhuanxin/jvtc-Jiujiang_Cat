...
class dormitory_knowing_State extends State<dormitory_knowing>{
  ...
  void _loading_dormitory_knowing_ui(){
    ...
    if(dormitory_knowing_list.length>10){
      sum_page=(dormitory_knowing_list.length/10);
      sum_page=int.parse(sum_page.toString().split('.')[0]);
      if(dormitory_knowing_list.length%10!=0){
        sum_page++;
      }
    }else{
      sum_page=1;
          ...
          children: <Widget>[
            new Row(
              children: <Widget>[
                ...
                Expanded(
                  flex: 1,
                  child: new Text(tempmap==null?'':tempmap['dorm'],textAlign: TextAlign.center,overflow: TextOverflow.ellipsis,),
                ),
              ],
            ),
            new Row(
            ...
            new Row(
              children: <Widget>[
                ...
                Expanded(
                  flex: 1,
                  child: new Text(tempmap==null?'':tempmap['source'],textAlign: TextAlign.center,overflow: TextOverflow.ellipsis,),
                ),
              ],
            ),
            new Row(
              children: <Widget>[
                ...
                Expanded(
                  flex: 1,
                  child: new Text(tempmap==null?'':tempmap['time'],textAlign: TextAlign.center,overflow: TextOverflow.ellipsis,),
                ),
                new Text('周次',textAlign: TextAlign.center),
                Expanded(
                  flex: 1,
                  child: new Text(tempmap==null?'':tempmap['week'],textAlign: TextAlign.center,overflow: TextOverflow.ellipsis,),
                ),
              ],
            ),
          ],
  ...
  void _get_Activity_evaluation() async{
    ...
    String str1=await HttpUtil.xg_query_activity_info('StuEnlightenRoomScore', prefs.getString('xgtoken'));
    if(int.parse(json.decode(str1)['code'].toString())==0){
      //Successful request , token vaild
      dormitory_knowing_list=json.decode(json.encode(json.decode(str1)['data']));
